...
func extractOperationDetails(operation xdr.Operation, transaction ingest.LedgerTransaction, operationIndex int32, network string) (map[string]interface{}, error) {
	...
	case xdr.OperationTypeManageBuyOffer:
		...
		details["amount"] = ConvertStroopValueToReal(op.BuyAmount)
	...
	case xdr.OperationTypeChangeTrust:
		...
		details["limit"] = ConvertStroopValueToReal(op.Limit)
	...
	case xdr.OperationTypeLiquidityPoolDeposit:
		...
		details["reserve_a_max_amount"] = ConvertStroopValueToReal(op.MaxAmountA)
	...
	case xdr.OperationTypeLiquidityPoolWithdraw:
		...
		details["reserve_a_min_amount"] = ConvertStroopValueToReal(op.MinAmountA)
		details["reserve_a_withdraw_amount"] = ConvertStroopValueToReal(receivedA)

		// Process AssetB Details
		if err := addAssetDetailsToOperationDetails(details, assetB, "reserve_b"); err != nil {
			return details, err
		}
		details["reserve_b_min_amount"] = ConvertStroopValueToReal(op.MinAmountB)
		details["reserve_b_withdraw_amount"] = ConvertStroopValueToReal(receivedB)

		details["shares"] = ConvertStroopValueToReal(op.Amount)
